...
def output_mask_txt(image_array: np.ndarray):
    ...
    np.savetxt(
        "output_image_mask.txt",
        tuple_array_with_brackets,
        fmt="%s",
        delimiter="\t",
        newline="\n",
    )
...
def vos_tracking_video(video_state, interactive_state, mask_dropdown):
    ...
    video_output, score_dict = crop_select(
        images_path, output_crop_path, bbx_path, model_path, video_path
    )
    sorted_items = sorted(score_dict.items(), key=lambda x: x[1], reverse=True)
    # 按得分大小，选出key
    sorted_keys = [
        f"{item[0].split('_')[0]}:{item[0].split('_')[1]} 得分{round(item[1], 2)}"
...
with gr.Blocks() as iface:
    ...
    ratio_prompt.change(fn=get_choices, inputs=ratio_prompt, outputs=video_output)
