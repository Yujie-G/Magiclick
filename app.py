...
def add_multi_mask(video_state, interactive_state, mask_dropdown):
    ...
    return interactive_state, gr.update(choices=interactive_state["multi_mask"]["mask_names"], value=mask_dropdown, visible=False), select_frame, [[],[]], operation_log
...
def remove_multi_mask(interactive_state, mask_dropdown):
    ...
    return interactive_state, gr.update(choices=[],value=[], visible=False), operation_log
...
def output_images(images:list):
    ...
    if not os.path.exists(full_path):
        ...
        clear_folder("ori_images")

    for image_array in images:

        the_frame = torch.from_numpy(image_array.transpose((2, 0, 1)))
        file_name = "ori_images/{:04d}.jpg".format(cnt)
        torchvision.io.write_jpeg(the_frame, filename=file_name)

        cnt += 1



def output_mask_txt(image_array:np.ndarray):
    # 将3D数组转换为2D数组
    flattened_array = image_array.reshape(image_array.shape[0], -1, 1)

    # 将每个元组都加上一对括号
    tuple_array = [tuple(row) for row in flattened_array]
    tuple_array_with_brackets = [f"({', '.join(map(str, tpl))})" for tpl in tuple_array]

    # 使用savetxt将带括号的元组保存到txt文件
    np.savetxt("output_image_mask.txt", tuple_array_with_brackets, fmt="%s", delimiter="\t", newline="\n")
...
def get_bounding_box(masks):
    ...
    for mask in masks:
        ...
        sorted_bbx = sorted(bounding_boxes, key=compare_tuples, reverse=True)

        x, y, w, h = sorted_bbx[0]
        res.append((x, y, w, h))
    ...
    with open("bounding_boxes.txt", "w") as file:
        cnt = 1
        for temp in res:
            line = "{:04d} {:d} {:d} {:d} {:d}".format(cnt, temp[0], temp[1], temp[2], temp[3])
            file.write(line + "\n")
        ...
        #     x, y, w, h = bbox
        #     line = f"Bounding Box: (x={x}, y={y}, width={w}, height={h})"
        #     file.write(line + "\n")
...
def vos_tracking_video(video_state, interactive_state, mask_dropdown):
    ...
    print("following type is", type(following_frames))
    output_images(following_frames)
    ...
    get_bounding_box(video_state["masks"])
    ...
    video_output, score_dict = crop_select(images_path, output_crop_path, bbx_path, model_path, video_path)
    sorted_items = sorted(score_dict.items(), key=lambda x: x[1], reverse=True)
    # 按得分大小，选出key
    sorted_keys = [f"{item[0].split('_')[0]}:{item[0].split('_')[1]} 得分{round(item[1], 2)}" for item in sorted_items]

    # sorted_keys = ["3_2 得分100", "16_9 得分90"]
    ...
    #### shanggao code for mask save
    ratio_prompt = gr.update(visible=True, choices=sorted_keys, value=sorted_keys[0])
    return video_output, video_state, interactive_state, operation_log, ratio_prompt
...
def get_choices(choice):
    ...
    left, right = choice.split(" ")[0].split(":")
    left = int(left)
    right = int(right)

    choice = f"{left}_{right}"

    path = f"result/track{choice}.mp4"
    print("Choose Video", path)
...
title = """<p><h1 align="center">Magiclick</h1></p>
    """
...
with gr.Blocks() as iface:
        ...
        with gr.Column():
            ...
            with gr.Row(scale=0.3):
                with gr.Column(align="center"):
                    ...
                    video_input = gr.Video(autosize=True)
                with gr.Column(align="center"):
                    video_output = gr.Video(autosize=True)
                ...
                with gr.Column():
                    ...
                    with gr.Row():
                        with gr.Row():
                                ...
                                tracking_video_predict_button = gr.Button(value="Magiclick", visible=False)
    ...
    tracking_video_predict_button.click(
        ...
        inputs=[video_state, interactive_state, mask_dropdown],
        outputs=[video_output, video_state, interactive_state, run_status, ratio_prompt]
    )
    ...
    gr.Examples(
        examples=[os.path.join(os.path.dirname(__file__), "./test_sample/", test_sample) for test_sample in ["girl.mp4","test-sample4.mp4", \
